export default async (request: Request, context: any): Promise<Response> => {
  ...
  const targetPath = url.pathname.replace('/realms/', '');
  const targetUrl = `https://casino-citizen.eks-dev01.gigndvr.com/realms/${targetPath}${url.search}`;
  ...
  try {
    ...
    console.error('Realms proxy error:', error);
